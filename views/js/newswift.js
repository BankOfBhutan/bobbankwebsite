...
const swift = async(service,Name,email,accountNumber,contact,swiftDate,swiftTime)=>{
    try {
        const conflictCheck = await axios.post('https://bankofbhutan-w3qb.onrender.com/api/v1/swift/check_Conflict', { email, swiftDate, swiftTime });
        ...
        await axios.post('https://bankofbhutan-w3qb.onrender.com/api/v1/swift/send-otp', { email });
        ...
        const res = await axios({
            method:'POST',
            url : 'https://bankofbhutan-w3qb.onrender.com/api/v1/swift/swift',
            data:{
                service,
                Name,
                email, 
        ...
        console.log(res)
        if (res.data.message === 'swift Token successfully booked!'){
...
document.querySelector('form').addEventListener('submit',(e)=>{
    ...
    const swiftDate = document.getElementById('Date').value 
    ...
    if (!Name || !email || !accountNumber || !contact || !swiftDate || !swiftTime) {
    ...
    if (!isValidTime) {
        errorshowModal('swift appointments are not allowed during lunch hours (1:00 PM - 2:00 PM) and off-hours (5:00 PM - 8:59 AM).');
        return;
    }

    if(swiftDate === today || swiftDate<today ){
        errorshowModal('swift appointments cannot be created for today and Past.');
        return;
    }
    ...
    swift(service,Name,email,accountNumber,contact,swiftDate,swiftTime)
