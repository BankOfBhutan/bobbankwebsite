...
const fetchTotalwaitingTokenCount = async () => {
    try {
        ...
        if (!tellerDetails || !tellerDetails.service) {
            ...
            document.getElementById('waiting-token').innerText = '0';
            return;
        }
        ...
        const response = await fetch(`https://bankofbhutan-w3qb.onrender.com/api/queue/waiting-token?serviceName=${encodeURIComponent(serviceName)}`, {
            method: 'GET',
        ...
        if (response.ok) {
            ...
            document.getElementById('waiting-token').innerText = data.tokenCount;
        } else {
...
document.addEventListener('DOMContentLoaded', () => {
    fetchTotalwaitingTokenCount();
    setInterval(fetchTotalwaitingTokenCount, 10000);
});